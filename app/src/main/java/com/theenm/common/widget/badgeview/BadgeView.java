package com.theenm.common.widget.badgeview;
...
public class BadgeView extends View
{
    ...
    protected void onDraw(Canvas canvas)
    {
        ...
        {
        case SHAPE_CIRCLE:
            canvas.drawCircle(getMeasuredWidth() / 2f, getMeasuredHeight()
                    / 2f, getMeasuredWidth() / 2, backgroundPaint);
            canvas.drawText(showText, getMeasuredWidth()
                    / 2f, getMeasuredHeight() / 2f
                            + (textH / 2f - fontMetrics.descent), numberPaint);
            break;
        case SHAPE_OVAL:
        ...
        case SHAPE_SQUARE:
            ...
            canvas.drawText(showText, sideLength / 2f, sideLength / 2f
                    + (textH / 2f - fontMetrics.descent), numberPaint);
            break;
     ...
     * @return
     */
    public BadgeView setWidthAndHeight(float w, float h)
    {
        FrameLayout.LayoutParams params = (FrameLayout.LayoutParams) getLayoutParams();
        params.width = dip2px(getContext(), w);
        params.height = dip2px(getContext(), h);
     ...
     *
     * @param horitontal
     *            horitontal space unit dp
     * @param vertical
     *            vertical space unnit dp
     * @return
     */
    ...
    public BadgeView bind(View view)
    {
        ...
        else if( view.getParent() instanceof ViewGroup )
        {
            ...
            {
                containerParams.height = origionHeight + topMargin
            ...
            {
                containerParams.width = origionWidth + rightMargin
                        + horiontalSpace + leftMargin;
            }
            container.setLayoutParams(containerParams);

            // setGravity
            FrameLayout.LayoutParams params = (FrameLayout.LayoutParams) getLayoutParams();
            if( params.gravity == (Gravity.RIGHT | Gravity.TOP)
            ...
            else if( params.gravity == (Gravity.LEFT | Gravity.TOP)
    ...
    public boolean unbind()
    {
        if( getParent() != null )
        {
            ...
            // ((ViewGroup)getParent()).getLayoutParams();
            // layoutParams.width = layoutParams.width - leftMargin -
            // rightMargin;
            // layoutParams.height = layoutParams.height -topMargin -
            // bottomMargin;
